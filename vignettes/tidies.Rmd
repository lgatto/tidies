...
package. `tidies` isn't part of the `tidyverse`, but aims at
...
library("tidyr")
x <- tidyr::gather(x, sample, exprs, -fv)
head(x)
```
...
metadata. This is the general principal that is applied, internally,
in the `tidies` functions.
